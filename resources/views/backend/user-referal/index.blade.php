...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">User Referal</li>
    </ol>
    <div class="row">
        <div class="col-lg-12">
            ...
            <table class="table table-striped datatables table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="desktop">Kode Referal</th>
                        <th class="desktop">Referal By User</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($users as $index => $user)
                        <tr @if(isset($user->group) && isset($user->group->role) &&  ($user->group->role->id == 1 && auth()->user()->group->role->id != 1)) class="overlay-permission" @endif @if($user->ban) class="overlay-banned" @endif>
                            ...
                            <td class="desktop">
                                <a href="{{ route('backend.user.referal.edit', ['id' => $user->id]) }}" class="row-button edit"><i class="fas fa-fw fa-edit"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
