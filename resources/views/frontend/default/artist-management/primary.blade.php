...
@section('pagination')
    @foreach ($participants as $par)
        <tr class="module" data-toggle="contextmenu" data-trigger="right" data-type="artists" data-id="{{ $par->id }}">
            ...
            <td class="text-center">{{ $my_artist[$par->artist_role-1]['name'] }}</td>
        </tr>
    @endforeach
@stop
@section('content')
    ...
    <div id="page-content">
        <div class="container">
            <div class="page-header artist main small desktop"> <a class="img "> <img src="{{ $artist->artwork_url }}" alt="{{ $artist->name}}">  </a>
                <div class="inner">
                    <h1 title="{!! auth()->user()->name; !!}">{!! auth()->user()->email; !!}<span class="subpage-header"> / Primary Artists</span></h1>
                    <div class="byline">Your Primary Artist lists</div>
                </div>
            </div>
                    ...
                    @if(count($participants))
                        <div class="card shadow">
                            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                <h2 class="m-0 font-weight-bold" data-translate-text="Artists">Primary Artists</h2>
                            </div>
                            <div class="card-body">
