...
class DashboardController
{
    public function index(Request $request)
    {
        ...
        $dashboard->total_albums_paid = DB::table('albums')->where('paid', 1)->count();
        $dashboard->total_albums_unpaid = DB::table('albums')->where('paid', 0)->count();
        $dashboard->total_albums_single = DB::table('albums')->where('type', 4)->count();
        $dashboard->total_albums_ep = DB::table('albums')->where('type', 5)->count();
        $dashboard->total_albums_lp = DB::table('albums')->where('type', 6)->count();
        $dashboard->total_today_albums = DB::table('albums')->whereDate('inserted_at', '=', Carbon::today())->count();
        $dashboard->total_today_albums_paid = DB::table('albums')->whereDate('inserted_at', '=', Carbon::today())->where('paid', 1)->count();
        $dashboard->total_yesterday_albums = DB::table('albums')->whereRaw('DATE(inserted_at) = DATE(CURDATE() - INTERVAL 1 DAY)')->count();
        $dashboard->total_yesterday_albums_paid = DB::table('albums')->whereRaw('date(inserted_at) = CURDATE() - INTERVAL 1 DAY')->where('paid', 1)->count();
        $dashboard->total_playlists = DB::table('playlists')->count();
        $dashboard->total_manual_paid = Transaction::withoutGlobalScopes()->where('payment_type', 'Manual')->sum('amount');
        $dashboard->total_qris_paid = Transaction::withoutGlobalScopes()->where('payment_type', 'QRIS')->sum('amount');
        $dashboard->total_free_voucher = Transaction::withoutGlobalScopes()->where('nilai_voucher','>','0')->sum('nilai_voucher');
        $dashboard->total_free_song = Transaction::withoutGlobalScopes()->where('nilai_free_song','>','0')->sum('nilai_free_song');
        ...
        $dashboard->statistics->total_users_has_album = User::whereIn('id', function($query){ $query->select('user_id')->from('albums'); })->count();
