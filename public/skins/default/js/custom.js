 ...
 **
 * Authors: NextArt NextArt2510@gmail.com
 * Web: http://NextArt.id
 *
...
(function (factory) {
        ...
        var slider = new Swiper(".swiper-container-slide", {
          ...
          navigation: {
            nextEl: $(".swiper-container-slide")
              ...
              .find(".swiper-arrow-right"),
            prevEl: $(".swiper-container-slide")
              .parents(".home-section")
              .find(".swiper-arrow-left"),
          },
        });
      }, 500);
    },
  ...
  window.DragQueue = {
      ...
      if (!$.engineUtils.isMobile()) {
        ...
        $("#embed_list_middle").sortable({
          ...
          start: function (e, ui) {
            ...
            ui.item.show();
            if (
              !$(".sortable-queue-helper").hasClass("ui-draggable-dragging")
            ) {
          ...
          change: function (e, ui) {
            ...
            $("#queueSortableTextHelper").css({
              ...
              left:
                $("#embed_list_middle .queue-sortable-placeholder").offset()
                  .left - 32,
            });
          },
          update: function (e, ui) {
            ...
            if (oldIndex) {
              ...
              $("#embed_list_middle li").each(function (index) {
                ...
                $(this)
                  .find(".embed_current_playlist_play_button")
                  .attr("id", "embed_current_playlist_play_button_" + index);
                EMBED.PlaylistObj.obj[
                  $(this).attr("id").replace("embed_current_playlist_row_", "")
                ].position = index;
              });
            } else {
    ...
    objectDropped: function (ui, position, forcePlay) {
      ...
      if (object_type === "song") {
        if (EMBED.Playlist.length) {
          var currentHash = hex_md5(
            EMBED.Playlist[EMBED.Player.queueNumber].stream_url
          );
          EMBED.PlaylistObj.addSongAtPosition(
            [$.engineUtils.toPlayerJson(object)],
            position
          );
          EMBED.Player.queueNumber =
            EMBED.PlaylistObj.obj[currentHash].position;
        } else {
          EMBED.PlaylistObj.addSongAtPosition(
            [$.engineUtils.toPlayerJson(object)],
            position
        ...
        setTimeout(function () {
          ...
          Toast.show(
            "queue",
            Language.text.POPUP_QUEUE_SONG_ADDED.replace(":numSongs", 1)
          );
          if (forcePlay) {
        ...
        Playlist.playLiveRadioStation(
          [$.engineUtils.stationToPlayerJson(object)],
          true
        );
        ...
        $.ajax({
          type: "get",
          url: route.route("api.user.recent", { id: object_id }),
          success: function (response) {
            if (response && response.songs && response.songs.data.length) {
              ...
              Toast.show(
                "queue",
                response.songs.data.length === 1
                  ? Language.text.POPUP_QUEUE_SONG_ADDED.replace(
                      ":numSongs",
                      response.songs.data.length
                    )
                  : Language.text.POPUP_QUEUE_SONGS_ADDED.replace(
                      ":numSongs",
                      response.songs.data.length
                    )
              );
        ...
        $.ajax({
          type: "get",
          url: route.route("api." + object_type, { id: object_id }),
          success: function (response) {
  ...
  window.Favicon = {
    ...
    icon: {
      play: {
        normal:
          route.route("frontend.homepage") + "common/images/play-favicon.png",
        retina:
          route.route("frontend.homepage") +
          "common/images/play-favicon@2x.png",
      },
      pause: {
        normal:
          route.route("frontend.homepage") + "common/images/pause-favicon.png",
        retina:
          route.route("frontend.homepage") +
          "common/images/pause-favicon@2x.png",
      },
    },
    watch: function () {
      ...
      EMBED.Player.Audio.addEventListener(
        "timeupdate",
        Favicon.onTimeUpdate,
        false
      );
      Favicon.done = true;
    },
    ...
    update: function () {
      ...
      if (Favicon.state === "play") {
        img.src = Favicon.isRetina()
          ? Favicon.icon.play.retina
          : Favicon.icon.play.normal;
      } else if (Favicon.state === "pause") {
        img.src = Favicon.isRetina()
          ? Favicon.icon.pause.retina
          : Favicon.icon.pause.normal;
      }
  ...
  var engineWaveform = {
    ...
    make: function () {
      ...
      if ($(".track-waveform").attr("data-uri")) {
        ...
        request.onload = function () {
          audioContext.decodeAudioData(
            ...
            function (buffer) {
              ...
              if ($(".track-waveform").hasClass("wave")) {
                ...
                halfWaveform(
                  document.getElementById(
                    "waveform-active-" +
                      $("#wave-form-container").attr("data-id")
                  ),
                  data,
                  "#e23036"
                );
              ...
              $.ajax({
                method: "POST",
                url: route.route("frontend.waveform.save", {
                  id: $("waveform").data("id"),
                }),
                data: {
                ...
                success: function (data) {
                  __DEV__ &&
                    console.log("Created and saved waveform perk data.");
                },
              });
            },
            function (error) {
        ...
        $.ajax({
          method: "GET",
          url: route.route("frontend.waveform.get", {
            id: $("waveform").data("id"),
          }),
          success: function (data) {
      ...
      try {
        ...
        if ($(".track-waveform").hasClass("wave")) {
          halfWaveform(
            document.getElementById("waveform"),
            engineWaveform.data,
            document.body.classList.contains("dark-theme") ? "#fff" : "#000"
          );
          halfWaveform(
            document.getElementById(
              "waveform-active-" + $("#wave-form-container").attr("data-id")
            ),
            engineWaveform.data,
            "#e23036"
          );
