...
(function (factory) {
  ...
  $.engineUtils = {
    ...
    getStorage: function (a) {
      ...
      if (((void 0 !== c && null !== c) || (c = 0), c < b))
        return $.engineUtils.removeStorage(a), null;
      try {
        ...
        return (
          console.log(
            "$.engineUtils.getStorage: Error reading key [" +
              a +
              "] from localStorage: " +
              JSON.stringify(b)
          ),
    ...
    setStorage: function (a, b, c) {
      ...
      try {
        sessionStorage.setItem(a, b),
          sessionStorage.setItem(a + "_expiresIn", e);
      } catch (b) {
        return (
          console.log(
            "$.engineUtils.setStorage: Error setting key [" +
              a +
              "] in localStorage: " +
              JSON.stringify(b)
          ),
    ...
    buttonProgress: function (percent) {
      ...
      var progressBorder = $(
        ".button-process-container[data-song-id=" +
          currentPlaying.id +
          "] .buttonProgressBorder"
      );
      var i;
      ...
      if (i <= 180) {
        ...
        progressBorder.css(
          ...
          "linear-gradient(" +
            (i - 90) +
            "deg, transparent 50%, #e23137 50%),linear-gradient(90deg, #fff 50%, transparent 50%)"
        );
      }
    },
    isMobile: function () {
          ...
          if ($.engineUtils.isMobile()) {
            if (navigator.userAgent.match(/(iPad|iPhone|iPod)/g)) {
              $(".context-menu-submenu").hover(function () {
                $(".context-menu-list:first").addClass(
