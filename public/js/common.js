...
(function (factory) {
  ...
  window.Toast = {
    ...
    icon: {
      ...
      check:
        '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg></svg>',
      favorite:
      ...
      follow:
        '<svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg>',
      radio:
    ...
    show: function (icon, msg, title) {
      ...
      var html =
        ...
        icon +
        '">' +
        '<a class="btn btn-icon-only btn-rounded btn-dark close">' +
        Toast.icon.close +
        "</a>" +
    ...
    resize: function (e) {
      $.engineSideBar.resizeHandle.draggable({
        ...
        start: function (event, ui) {
          ...
          $.engineSideBar.dragPositionTop =
            event.clientY - $.engineSideBar.playlistsHeight;
          $.engineSideBar.dragPositionBottom =
            $.engineSideBar.resizeHandle.height() -
            $.engineSideBar.dragPositionTop;
        },
        drag: function (event, ui) {
          $.engineSideBar.playlists.height(
            event.clientY - $.engineSideBar.dragPositionTop
          );
          $.engineSideBar.community.css(
            "top",
            event.clientY +
              $.engineSideBar.dragPositionBottom -
              $.engineSideBar.resizeHandle.height()
  ...
  window.Language = {
    ...
    set: function () {
      ...
      $("<span/>", {
        ...
        .html(Language.text.QUEUE_ITEM_OPTIONS)
        .appendTo("#queue-menu-btn");
      ...
      $("<span/>", {
        ...
        .html(Language.text.QUEUE_RESTORE_QUEUE)
        .appendTo("#embed_restore_icon");
      ...
      $("<span/>", {
        ...
        .html(Language.text.QUEUE_SOUND_VOLUME_MUTE)
        .appendTo("#embed_volume_speaker");
      ...
      $("<span/>", {
        ...
        .html(Language.text.QUEUE_SHUFFLE_SONGS)
        .appendTo("#embed_large_shuffle");
      ...
      $("<span/>", {
        ...
        .html(Language.text.QUEUE_LOOP_SONGS)
        .appendTo("#embed_large_repeat");
      ...
      $("<span/>", {
        ...
        .html(Language.text.QUEUE_TOGGLE)
        .appendTo("#embed_list_icon");
  ...
  window.Search = {
    ...
    suggest: {
      ...
      show: function (searchVal) {
        Search.currentRequest = $.ajax({
          type: "GET",
          url: route.route("api.search.suggest", {
            q: searchVal,
            limit: Search.suggest.limit,
          }),
          cache: true,
          beforeSend: function () {
          ...
          success: function (response) {
            ...
            html
              ? $(".tooltip.suggest").removeClass("hide")
              : $(".tooltip.suggest").addClass("hide");
          },
        });
      },
    ...
    didYouMean: function () {
      ...
      $.ajax({
        url:
          "https://google.com/complete/search?output=json&q=" +
          encodeURIComponent(term) +
          "%20lyrics&client=serp",
          ...
          try {
            if (response[1][0][0]) {
              ...
              if (
                ...
                link.html(suggest).attr(
                  "href",
                  route.route("frontend.search.song", {
                    slug: suggest,
                  })
                );
  ...
  function formatRepo(repo) {
    ...
    var markup =
      ...
      "<div class='select2-result-repository__artwork'><img src='" +
      repo.artwork +
      "' /></div>" +
  ...
  function formatRepoSelection(repo) {
    var markup =
      ...
      "' /></div>" +
      "<div class='select2-result-repository__meta'>" +
      "<div class='select2-result-repository__title white'>" +
      repo.title +
      "</div></div>";
  ...
  window.Settings = {
    ...
    profile: {
      update: function (response, $form) {
        $("img.profile-img, img.user-picture-preview").attr(
          "src",
    ...
    account: {
      update: function (response, $form) {
        $form
          .find("[name='settings-email']")
          .closest(".control")
          .removeClass("field-error");
        $form
          .find("[name='settings-password']")
          .closest(".control")
          .removeClass("field-error");
        $form
          .find("[type='submit']")
  ...
  AjaxForm.init = function () {
    $(".ajax-form").ajaxForm({
      beforeSubmit: function (data, $form, options) {
        if (
          !User.isLogged() &&
          $form.attr("id") !== "singup-form" &&
          $form.attr("id") !== "forgot-form" &&
          $form.attr("id") !== "reset-password-form"
        ) {
          $.engineLightBox.hide();
          User.SignIn.show();
          return false;
      ...
      success: function (response, textStatus, xhr, $form) {
        ...
        if (form === "settings-profile-form") {
          ...
          Toast.show(
            "success",
            Language.text.POPUP_CHANGED_PASSWORD_SUCCESS_DESCRIPTION,
            Language.text.POPUP_CHANGED_PASSWORD_SUCCESS_TITLE
          );
        } else if (form === "stripe-form") {
          console.log(
            $form.attr("action"),
            route.route("frontend.stripe.purchase")
          );
          if (
            $form.attr("action") === route.route("frontend.stripe.purchase")
          ) {
      ...
      error: function (e, textStatus, xhr, $form) {
        ...
        if (e.status === 429) {
          ...
          $form
            ...
            .removeClass("hide")
            .html(
              e.responseJSON.errors[Object.keys(e.responseJSON.errors)[0]][0]
            );
          $form
            .find("[name='" + Object.keys(e.responseJSON.errors)[0] + "']")
            .closest(".control")
            .addClass("field-error");
        }
        $form
  ...
  window.Filter = {
    ...
    show: function (term) {
      var listItems = $(
        "#songs-grid > div, #user-profile-grid > div, .items-sort-able > div, .items-filter-able > div"
      );
  ...
  window.Download = {
    init: function (song) {
      ...
      if (User.isLogged()) {
        el.find(".download-tip-learn").attr(
          "href",
          route.route("frontend.settings.subscription")
        );
      } else {
      ...
      if (parseInt(song.mp3) && parseInt(song.hd)) {
        ...
        if (Boolean(song.allow_high_quality_download)) {
          el.find(".hq-download").bind("click", function () {
            window.open(
              route.route("frontend.song.download.hd", { id: song.id })
            );
          });
                ...
                success: function (response) {
                  if (response.genres && response.genres.length) {
                    response.genres.forEach(function (i) {
                      ...
                      tag.attr(
                        "href",
                        route.route("frontend.genre", { slug: i.alt_name })
                      );
                      tag.html(i.name);
                      $("#" + el_id)
                        .find(".tags-wrapper")
  ...
  window.BrowseStation = {
    init: function () {
      ...
      $(".toolbar-country-filter-select2").bind("change", function () {
        ...
        if (country_code) {
          $.post(
            route.route("frontend.radio.get.city"),
            {
          ...
          $.post(
            route.route("frontend.radio.get.language"),
            {
              countryCode: country_code,
            },
            function (data) {
    ...
    build: function () {
      ...
      window.history.pushState(
        ...
        url,
        window.location.href.split("?")[0] + "?" + query + "&browsing=true"
      );
  ...
  window.GlobalReport = {
    ...
    popup: function (type, id) {
      ...
      var problems = [];
      if (type === "podcast") {
        problems = Language.text.PODCAST_REPORT
          ? Language.text.PODCAST_REPORT
          : [];
      } else if (type === "episode") {
        problems = Language.text.PODCAST_EPISODE_REPORT
          ? Language.text.PODCAST_EPISODE_REPORT
          : [];
      } else if (type === "song") {
        ...
        callback: function (result) {
          if (result) {
            ...
            Report.init(
              reportable_type,
              id,
              problems[parseInt(result) - 1].text,
              Language.text.SUCCESS_REPORT_PROBLEMS
            );
  ...
  $(window).on("enginePageHasBeenLoaded", function () {
    ...
    $('[data-content="snapshot-search"]').each(function () {
      var model = $(this);
      $.ajax({
        url: route.route("frontend.search." + model.attr("data-type"), {
          slug: model.attr("data-keyword"),
        }),
        data: {
          limit: 3,
        },
        ...
        success: function (response) {
          if (response && response.data.length) {
            ...
            for (var i = 0; i < num; i++) {
              ...
              window[model.attr("data-type") + "_data_" + response.data[i].id] =
                response.data[i];
            }
          }
        },
  ...
  $(document).on("change", ".input-group [type=file]", function () {
    ...
    $(this)
      .next()
      .val(fileName.replace(/C:\\fakepath\\/i, ""));
  });
  ...
  if (!$.engineUtils.isMobile()) {
    ...
    var playlistBarHeight =
      parseInt($("#sideMenu").height()) -
      parseInt($("#aside_ul").height()) -
      80;
