      ...
      if (!a.attr("target")) {
        if (href) {
          if (a.hasClass("show-song-tooltip") === true) {
            ...
            if ($.engineUtils.isMobilfe()) {
              ...
              if (song && song.id !== undefined) {
                el.find(".song-link:not(.image)")
                  .html(song.title)
                  .attr("data-song-id", song.id)
                  .attr("href", song.songurl);
                el.find(".artist-link")
                  .html(song.artistname)
                  .attr("href", song.artisturl);
                el.find(".album-link")
                  .html(song.albumname)
                  .attr("href", song.albumurl);
          ...
          if (a.hasClass("close") === true) {
            ...
            view === "grid"
              ? $(target).addClass("grid-view")
              : $(target).removeClass("grid-view");
    ...
    if (getCache === null || getCache === undefined) {
      $.ajax({
        ...
        success: function (response) {
          ...
          if (
            route
              .route("frontend.settings.preferences")
              .replace(route.route("frontend.homepage"), "") !== a.href &&
            route
              .route("frontend.auth.user.artist.manager.uploaded")
              .replace(route.route("frontend.homepage"), "") !== a.href &&
            route
              .route("frontend.cart")
              .replace(route.route("frontend.homepage"), "") !== a.href
...
$(function () {
  $(window).on(
    "engineHistoryChange",
    $.engineBrowseHistory.engineHistoryChange
  );
